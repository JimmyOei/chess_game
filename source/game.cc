...
void Game::renderBoard() {
    ...
    // Takes the largest of screenHeight and screenWidth
    int rectEdge = screenHeight * (screenHeight < screenWidth) 
                   + screenWidth * (screenHeight >= screenWidth);

    int boardStartingX = (screenWidth - rectEdge) / 2;
    int boardStartingY = (screenHeight - rectEdge) / 2;
...
void Game::resizeWindow(int const width, int const height) {
    ...
    screenWidth = width * (width > MIN_SCREEN_WIDTH) 
                          + MIN_SCREEN_WIDTH * (height <= MIN_SCREEN_WIDTH);
    Game::renderBoard();
}